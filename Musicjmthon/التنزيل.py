...
@Client.on_message(filters.command(["تحميل", "حمل"], prefixes=f"{HNDLR}"))
async def song(client, message: Message):
    urlissed = get_text(message)
    if not urlissed:
        await client.send_message(
            message.chat.id,
            "حط اسم الاغنيه متقرفنيش !",
        )
        return
    pablo = await client.send_message(message.chat.id, f"**بحملك اهو استني يعم .** `{urlissed}`")
    ...
    try:
        ...
        await pablo.edit(f"**ف خطأ يسطا اتفضل صلحو . ** \n**خطأ :** `{str(e)}`")
        return
    c_time = time.time()
    capy = f"""
...
"""
    ...
    await client.send_audio(
        ...
        progress_args=(
            pablo,
            c_time,
            f"**بحملك اهو استني يعم .** `{urlissed}`",
            file_stark,
        ),
    )
    await pablo.delete()
...
async def vsong(client, message: Message):
    ...
    try:
        ...
        await event.edit(event, f"**معرفتش انزلك .** \n**خطأ :** `{str(e)}`")
        return
    c_time = time.time()
