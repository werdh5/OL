...
@Client.on_message(filters.command(["play", "تشغيل"], prefixes=f"{HNDLR}"))
async def play(client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await m.delete()
            huehue = await replied.reply("**🔄 من فضلك انتظر .. جاري التشغيل !**")
...
""",
        ...
        if len(m.command) < 2:
            await m.reply("حط يعم اسم الاغنيه متقرفنيش .")
        else:
            await m.delete()
            huehue = await m.reply("استني يعم ببحثلك اهو .")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            if search == 0:
                await huehue.edit("ملقتش الاغنيه شوف واحده غيرها .")
            else:
                songname = search[0]
                url = search[1]
                duration = search[2]
                ...
                if hm == 0:
                    await huehue.edit(f"**ف خطأ يسطا اتفضل صلحو .** \n\n`{ytlink}`")
                else:
                    if chat_id in QUEUE:
...
async def vplay(client, m: Message):
    ...
    if replied:
        if replied.video or replied.document:
            await m.delete()
            huehue = await replied.reply("**بشغلك الفديو ي عم 😉**")
            dl = await replied.download()
            link = replied.link
            if len(m.command) < 2:
                Q = 720
